(** [Enclosure] module follows [Faraday] implementation (thx \@spiros). The main
    difference is: [Enclosure] uses a bounded internal [Bstr.t] which can not be
    grow. Internal queue which contains [IOVec] elements can not grow too.
...
val schedule_bigstring : encoder -> ?off:int -> ?len:int -> Bstr.t -> encoder
...
(** [kschedule_bigstring k t ?off ?len a]: [k]ontinuation-style of
    {!schedule_bigstring}. *)
...
(** [kschedule_string k t ?off ?len a]: [k]ontinuation-style of
    {!schedule_string}. *)
...
(** [kschedule_bytes k t ?off ?len a]: [k]ontinuation-style of
    {!schedule_bytes}. *)
...
val write_bigstring : ?off:int -> ?len:int -> Bstr.t -> encoder -> encoder
...
(** [create ~emitter len] creates a serializer with a fixed-length internal
    buffer. When internal buffers are full, [encoder] calls [emitter] to emit
    [IOVec.t list]. *)
