...
let lift () =
  ...
  let sep ppf () = let open Prettym in eval ppf [ spaces 1; ] in
  let pp ppf lst =
    let open Prettym in
    eval ppf [ !!(list ~sep:(sep, ()) string) ] lst in
  let lst = List.init 500 (fun _ -> random_word 14) in
  let res = Prettym.to_string ~margin:78 pp lst in
  let check line = Alcotest.(check bool) "< 80" (String.length line < 80) true in
